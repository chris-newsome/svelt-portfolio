	...
	<div class="mx-auto w-full max-w-7xl">
		<div class="grid min-h-[65vh] grid-cols-1 items-center md:grid-cols-2">
			...
			<div class="col-start-1 md:row-start-1">
				<h1 class="mb-2 md:mb-2 text-[clamp(3rem,18vmin,13rem)] font-extrabold leading-none tracking-tighter text-nowrap" aria-label={slice.primary.first_name + ' ' + slice.primary.last_name}>
					{#if first_name_letters.length && last_name_letters.length}
					<span class="block text-[#004a8f]"> <!-- Darker blue for first name -->
						{#each first_name_letters as letter}
							<span class="name-animation inline-block opacity-0">{letter}</span>
						{/each}
					</span>
					<span class="block text-[#34699a] -mt-[.2em]"> <!-- Slightly lighter blue for last name -->
				...
				<span class="job-title block bg-gradient-to-tr from-[#2f4f4f] via-[#34699a] to-[#004a8f] bg-clip-text text-transparent 
					text-2xl font-bold uppercase tracking-[.2em] md:text-4xl opacity-0">
